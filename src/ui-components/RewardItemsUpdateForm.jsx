...
import { getRewardItems } from "../graphql/queries";
import { updateRewardItems } from "../graphql/mutations";
export default function RewardItemsUpdateForm(props) {
  const {
    id: idProp,
    rewardItems: rewardItemsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = rewardItemsRecord
      ? { ...initialValues, ...rewardItemsRecord }
      : initialValues;
    setRewardName(cleanValues.RewardName);
    setRewardCost(cleanValues.RewardCost);
    setErrors({});
  };
  const [rewardItemsRecord, setRewardItemsRecord] =
    React.useState(rewardItemsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getRewardItems.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRewardItems
        : rewardItemsModelProp;
      setRewardItemsRecord(record);
    };
    queryData();
  }, [idProp, rewardItemsModelProp]);
  React.useEffect(resetStateValues, [rewardItemsRecord]);
  const validations = {
    RewardName: [],
    RewardCost: [],
  };
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateRewardItems.replaceAll("__typename", ""),
            variables: {
              input: {
                id: rewardItemsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RewardItemsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Reward name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || rewardItemsModelProp)}
