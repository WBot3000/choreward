...
import { getChanllenges } from "../graphql/queries";
import { updateChanllenges } from "../graphql/mutations";
export default function ChanllengesUpdateForm(props) {
  const {
    id: idProp,
    chanllenges: chanllengesModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = chanllengesRecord
      ? { ...initialValues, ...chanllengesRecord }
      : initialValues;
  ...
  const [chanllengesRecord, setChanllengesRecord] =
    React.useState(chanllengesModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getChanllenges.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getChanllenges
        : chanllengesModelProp;
      setChanllengesRecord(record);
    };
    queryData();
  }, [idProp, chanllengesModelProp]);
  React.useEffect(resetStateValues, [chanllengesRecord]);
  const validations = {
    ChallengeList: [],
    Family1Name: [],
    Family2Name: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateChanllenges.replaceAll("__typename", ""),
            variables: {
              input: {
                id: chanllengesRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ChanllengesUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || chanllengesModelProp)}
