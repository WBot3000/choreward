...
import { getRewards } from "../graphql/queries";
import { updateRewards } from "../graphql/mutations";
export default function RewardsUpdateForm(props) {
  const {
    id: idProp,
    rewards: rewardsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = rewardsRecord
      ? { ...initialValues, ...rewardsRecord }
      : initialValues;
    setRewardName(cleanValues.RewardName);
    setRewardCost(cleanValues.RewardCost);
    setErrors({});
  };
  const [rewardsRecord, setRewardsRecord] = React.useState(rewardsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getRewards.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRewards
        : rewardsModelProp;
      setRewardsRecord(record);
    };
    queryData();
  }, [idProp, rewardsModelProp]);
  React.useEffect(resetStateValues, [rewardsRecord]);
  const validations = {
    RewardName: [],
    RewardCost: [],
  };
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateRewards.replaceAll("__typename", ""),
            variables: {
              input: {
                id: rewardsRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "RewardsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Reward name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || rewardsModelProp)}
